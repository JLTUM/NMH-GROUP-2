...
grid = [10,15,20,25,30,35,40,50,70,100,1000,10000];
...
%     
    ...
    error(j,5) = int_er_U(n/5);
    error(j,6) = int_er_D(n/5);
    error(j,7) = int_er_C(n/5);
    
%     error(j,2) = er_U(n/2);
%     error(j,3) = er_D(n/2);
%     error(j,4) = er_C(n/2);
%     
%     error(j,5) = int_er_U(n/2);
%     error(j,6) = int_er_D(n/2);
%     error(j,7) = int_er_C(n/2);
...
figure
    loglog(error(:,1),error(:,2),'-x',error(:,1),error(:,3),'-x',error(:,1),error(:,4),'-x'...
        ,error(:,1),error(:,5),'-o',error(:,1),error(:,6),'-o',error(:,1),error(:,7),'-o');
    
    
    title('Error plot first derivative');
    xlabel('Grid spacing h');
    ylabel('Relative error');
    legend('Error Upwind','Error Downwind','Error Central',...
        'Error Upwind int.','Error Downwind Int','Error Central Int',...
        'Location','SouthEast')
