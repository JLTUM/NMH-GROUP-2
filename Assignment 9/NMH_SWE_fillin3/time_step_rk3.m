function [ run, flow3 ] = time_step_rk3( firstcall, constants, grid, run, ...
        flow3, bconds )
...
if firstcall
    ...
    dhdt  = zeros( [ size(flow3.h),  s] );
    dhudt = zeros( [ size(flow3.hu), s] );
    dhvdt = zeros( [ size(flow3.hv), s] );
    
    % Create arrays to store intermediate values of the 2D fields
    hstage  = zeros( size(flow3.h)  );
    hustage = zeros( size(flow3.hu) );
    hvstage = zeros( size(flow3.hv) );
end
...
for irk = 1:s
    ...
    [ dhdt(:,:,irk), dhudt(:,:,irk), dhvdt(:,:,irk)] = ...
        ...
        tstage, hstage, hustage, hvstage, flow3.kst, flow3.zb, ...
        dhdt(:,:,irk), dhudt(:,:,irk), dhvdt(:,:,irk) );
end
...
for j = 1:s
    flow3.h  = flow3.h  + dhdt(:,:,j) * b(j) * run.dt;
    flow3.hu = flow3.hu + dhudt(:,:,j) * b(j) * run.dt;
    flow3.hv = flow3.hv + dhvdt(:,:,j) * b(j) * run.dt;
end

% apply boundary conditions on the final fields
[flow3.h, flow3.hu, flow3.hv] = apply_boundary_conditions(constants, grid, bconds, flow3.h, flow3.hu, flow3.hv);
