...
for itstep = 1:run.ntst
    ...
    H_dynamic_energy = u.^2 / (2*constants.g) + flow.h(:,2);
...
figure(1)
 ...
 %   subplot(132)
 figure(2)
    plot(grid.x(2:end), H_dynamic_energy(2:end), grid.x(2:end), flow.h(2:end,2)+flow.zb(2:end,2),...
         grid.x(2:end),flow.h (2:end,2), grid.x(2:end), flow.zb(2:end,2),grid.x(2:end), v_h(2:end));
    title(['Head and Waterlevel',', timestep= ',num2str(itstep)])
